  ...
  static async getInstanceId(): Promise<string> {
    ...
    try {
      ...
      if (storedData[this.STORAGE_KEY]) {
        ...
        console.log(
          `[HTJ Background] Retrieved instance ID: ${this.instanceId}`
        );
        return this.instanceId!;
      }
    } catch (error) {
      console.error("[HTJ Background] Error retrieving instance ID:", error);
    }
    ...
    try {
      ...
      console.log(
        `[HTJ Background] Generated and saved new instance ID: ${newId}`
      );
    } catch (error) {
      console.error("[HTJ Background] Error saving new instance ID:", error);
    }
    return newId;
  }
}
